<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header title="Edit Alasan Penolakan" />
    </x-slot>

    <div class="py-6">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <x-ui.card>
                <form method="POST" action="{{ route('admin.rejection-reasons.update', $rejectionReason) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="label" class="block text-sm font-medium text-gray-700 mb-1">Label Alasan</label>
                        <input type="text" name="label" id="label" value="{{ old('label', $rejectionReason->label) }}"
