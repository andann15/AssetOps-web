<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header title="Tambah Unit Kerja" description="Tambahkan Unit Kerja baru ke dalam sistem.">
            <x-slot name="action">
    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <x-ui.card>
                <form method="POST" action="{{ route('admin.work-units.store') }}" class="space-y-5" x-data="{ newCompartment: false, newDepartment: false }">
                    @csrf
                    ...
                    <div>
                        ...
                        <div x-show="!newCompartment">
                            <select id="compartment_filter" name="compartment_id" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <option value="">-- Pilih Kompartemen --</option>
                                @foreach ($compartments as $comp)
                                    <option value="{{ $comp->id }}" {{ old('compartment_id') == $comp->id ? 'selected' : '' }}>{{ $comp->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div x-show="newCompartment" style="display: none;">
                            <input type="text" name="new_compartment_name" placeholder="Ketik nama kompartemen baru..." value="{{ old('new_compartment_name') }}" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('new_compartment_name') border-red-400 @enderror">
                    ...
                    <div>
                        ...
                        <div x-show="!newDepartment">
                            <select id="department_id" name="department_id"
                                class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('department_id') border-red-400 @enderror">
                                <option value="">-- Pilih Departemen --</option>
                                @foreach ($departments as $dept)
                                    <option value="{{ $dept->id }}" data-compartment="{{ $dept->compartment_id }}" {{ old('department_id') == $dept->id ? 'selected' : '' }}>
                                        {{ $dept->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div x-show="newDepartment" style="display: none;">
                            <input type="text" name="new_department_name" placeholder="Ketik nama departemen baru..." value="{{ old('new_department_name') }}" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('new_department_name') border-red-400 @enderror">
                    ...
                    {{-- Nama Unit Kerja --}}
                    <div>
                        <label for="name" class="block text-sm font-semibold text-gray-700 mb-1.5">Nama Unit Kerja <span class="text-xs text-gray-400 font-normal ml-1">(Opsional)</span></label>
                        <input type="text" id="name" name="name" value="{{ old('name') }}"
                            placeholder="Contoh: Unit Jaringan"
                            class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('name') border-red-400 @enderror">
